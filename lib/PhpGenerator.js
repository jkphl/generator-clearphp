...
module.exports = class extends Generator {
    ...
    prompting() {
        ...
        if (this._shouldRun('git')) {
            prompts.push({
                name: 'git',
                message: 'What\'s the Github repository URL for this project?',
                default: (answers) => this.config.get('git') || `https://github.com/${(answers.vendor || this.config.get('vendor')).toLowerCase()}/${(answers.project || this.config.get('project')).toLowerCase()}`,
                validate: function (url) {
                    return url.length ? (isGithubUrl(url) || 'The URL must be empty or a valid Github repository URL!') : true;
                },
                filter: function(url) {
                    const git = giturl(url);
                    return {
