...
function isGithubUrl(url) {
    try {
        ...
        return ((git.resource === 'github.com') && (git.owner.length > 0) && (git.name.length > 0)) ? {
...
module.exports = class extends Generator {
    ...
    prompting() {
        ...
        if (this._shouldRun('git')) {
            prompts.push({
                ...
                default: (answers) => (this.config.get('git') ? this.config.get('git').http : false) || `https://github.com/${(answers.vendor || this.config.get('vendor')).toLowerCase()}/${(answers.project || this.config.get('project')).toLowerCase()}`,
        ...
        if (this._shouldRun('codeclimate')) {
            prompts.push({
                ...
                when: a => typeof (a.git || this.config.get('git')) === 'object'
            });
        }
        ...
        return this.prompt(prompts).then((props) => {
            ...
            for (let d in inverseDependencies) {
    ...
    configuring() {
        ...
        // Load the Git configuration files
        if (this.config.get('git')) {
        ...
        if (this._shouldRun('git')) {
            this.composer = this._jsonConfig(this.composer, this.templatePath('git/configure/composer.json'));
        }
        ...
        if (this._shouldRun('apidocs')) {
            console.log('configuring apidocs');
    ...
    writing() {
        ...
        if (this._shouldRun('main', 'git', 'apidocs')) {
    ...
    install() {
        ...
        if (this._shouldRun('git')) {
            ...
            const git = this.config.get('git').ssh;
            if (git) {
                var that = this;
                var done = this.async();
     ...
     */
    _shouldRun(stage) {
        const shouldRun = Array.from(arguments).reduce((res, stageName) => {
