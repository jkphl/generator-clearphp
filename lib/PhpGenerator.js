...
module.exports = class extends Generator {
                ...
                exec('`which git` init', function (error, stdout, stderr) {
                    if (!error) {
                        ...
                        var setupGit = function () {
                            exec('`which git` remote add origin "' + git + '" && `which git` config core.filemode false', done);
                        };
     ...
     */
        ...
        if (this._shouldRun(this.stage)) {
            this.config.set('run', this.config.get('run') | this.runGenerators);
        }
    }
     ...
     *
     * @param {Sting} stage Run stage
     * @returns {Boolean} Whether this stage should be run
     */
    _shouldRun(stage) {
        const shouldRun = Array.from(arguments).reduce((s, r) => {
            let rval = r;
            let rvar = true;
            if (r.substr(0, 1) == '+') {
                rval = r.substr(1);
            ...
            const runIndex = rval ? generators.indexOf(rval) : -1;
            if ((runIndex >= 0) && (rvar || this._shouldRunVar(rval))) {
                return (s < 0) ? Math.pow(2, runIndex) : (s | Math.pow(2, runIndex));
            }
            return s;
        }, -1);
